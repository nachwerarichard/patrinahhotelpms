...
checkAvailabilityBtn.addEventListener('click', async () => {
    ...
    try {
        ...
        if (keys.length > 0) {
            keys.forEach((typeName, typeIndex) => {
                ...
                // Establish images array or fallback
                const images = (detail.imageUrls && detail.imageUrls.length > 0) 
                    ? detail.imageUrls 
                ...
                if (roomData.rooms.length > 0) {
                    ...
                    // Generate dynamic image slides
                    const imagesHtml = images.map(url => `
                        <img src="${url}" class="w-full h-48 object-cover flex-shrink-0 snap-center" alt="${typeName}">
                    `).join('');

                    // Render inner template containing the inner carousel elements
                    card.innerHTML = `
                        <div class="relative overflow-hidden group/carousel">
                            <div id="carouselTrack-${typeIndex}" class="flex overflow-x-auto scroll-smooth snap-x snap-mandatory no-scrollbar class-carousel-track" style="scrollbar-width: none; -ms-overflow-style: none;">
                                ${imagesHtml}
                            </div>
                            
                            ${images.length > 1 ? `
                                <button type="button" class="absolute left-2 top-1/2 -translate-y-1/2 bg-black/40 hover:bg-black/70 text-white p-1 rounded-full opacity-0 group-hover/carousel:opacity-100 transition-opacity duration-200" onclick="event.stopPropagation(); document.getElementById('carouselTrack-${typeIndex}').scrollLeft -= document.getElementById('carouselTrack-${typeIndex}').clientWidth">
                                    <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 19l-7-7 7-7" /></svg>
                                </button>
                                <button type="button" class="absolute right-2 top-1/2 -translate-y-1/2 bg-black/40 hover:bg-black/70 text-white p-1 rounded-full opacity-0 group-hover/carousel:opacity-100 transition-opacity duration-200" onclick="event.stopPropagation(); document.getElementById('carouselTrack-${typeIndex}').scrollLeft += document.getElementById('carouselTrack-${typeIndex}').clientWidth">
                                    <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7" /></svg>
                                </button>
                                <div class="absolute bottom-2 right-2 bg-black/60 text-white text-[10px] px-2 py-1 rounded-md backdrop-blur-sm pointer-events-none">
                                    1 / ${images.length} Photos
                                </div>
                            ` : ''}

                            <div class="absolute top-3 right-3 bg-white/90 backdrop-blur px-2 py-1 rounded-md shadow-sm pointer-events-none">
                    ...
                    // Optional: Setup indicator update hooks for modern scrolling tracking
                    if (images.length > 1) {
                        const track = card.querySelector(`#carouselTrack-${typeIndex}`);
                        const badge = card.querySelector('.absolute.bottom-2.right-2');
                        track.addEventListener('scroll', () => {
                            const pageIndex = Math.round(track.scrollLeft / track.clientWidth) + 1;
                            badge.textContent = `${pageIndex} / ${images.length} Photos`;
                        });
                    }
                }
            });
