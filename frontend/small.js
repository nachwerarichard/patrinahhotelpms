...
const updateActiveAccountUI = (account) => {
    ...
    // 1. Show "Complete Order" if there are uncommitted draft items
    if (completeOrderBtn) {
        completeOrderBtn.classList.toggle('hidden', !hasUncommittedCharges);
    }

    // 2. Show settlement options ("Post to Room" & "Settle Bill") ONLY after order completion (no uncommitted drafts remaining)
    const canSettle = hasAnyCharges && !hasUncommittedCharges;

    if (postToRoomBtn) {
        // Must be clear of uncommitted items AND have a valid room assigned
        postToRoomBtn.classList.toggle('hidden', !(canSettle && account.roomNumber));
    }

    if (issueReceiptBtn) {
        // Show Settle Bill only when all draft items have been committed
