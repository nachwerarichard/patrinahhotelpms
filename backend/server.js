...
// 📥 PESAPAL WEBHOOK - This updates the database AFTER payment confirmation
app.post('/api/public/pesapal-ipn', async (req, res) => {
    console.log("📥 [PESAPAL IPN RECEIVED] Confirming transaction status...");

    // Pesapal sends these parameters when a payment status alters
    const { OrderTrackingId, OrderMerchantReference, OrderNotificationType } = req.body;

    if (!OrderTrackingId) {
        return res.status(400).json({ message: "Missing tracking identification payload." });
    }

    try {
        // 1. Locate the pending booking using Pesapal's unique tracking ID
        const booking = await Booking.findOne({ transactionid: OrderTrackingId });
        
        if (!booking) {
            console.error(`❌ No booking matches Tracking ID: ${OrderTrackingId}`);
            return res.status(404).json({ message: "Booking record missing." });
        }

        // 2. Fetch the hotel credentials to authenticate the validation handshake
        const gatewaySettings = await Gateway.findOne({ hotelId: booking.hotelId, gatewayId: 'pesapal' });
        const isLive = gatewaySettings.environment === 'Live';

        // 3. Authenticate with Pesapal to get a temporary access token
        const authUrl = isLive 
            ? 'https://pay.pesapal.com/v3/api/Auth/RequestToken' 
            : 'https://cybqa.pesapal.com/pesapalv3/api/Auth/RequestToken';
        ...
        // 4. Request the official payment status from Pesapal directly
        ...
        // 5. Check if the status is completed (Pesapal status_code 1 means Completed)
        if (paymentData.status_code === 1 || paymentData.payment_status_description === 'Completed') {
            console.log(`✅ Payment verified for Booking: ${booking.id}. Updating database metrics...`);

            // Securely modify parameters now that cash has cleared
            booking.amountPaid = booking.totalDue; 
            booking.balance = 0;
            booking.paymentStatus = 'Paid';
            booking.gueststatus = 'confirmed'; // Elevate from 'reserved' to 'confirmed'

            await booking.save();
            console.log(`💾 Booking ${booking.id} updated to PAID successfully.`);
        } else {
            console.log(`⚠️ Transaction status update received but status is: ${paymentData.payment_status_description}`);
        }

        // 6. Return the exact response format Pesapal expects to stop webhook retries
        res.status(200).json({
            OrderTrackingId: OrderTrackingId,
            Status: "NEW", 
            Message: "Notification handled securely."
        });

    } catch (error) {
        console.error("💥 Critical Failure updating booking within Webhook pipeline:", error);
