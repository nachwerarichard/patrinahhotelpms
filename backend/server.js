...
await addAuditLog(
    ...
    { 
        ...
        // Update Room Booking Financial Metrics
...
app.post('/api/ai/manager-chat', auth, async (req, res) => {
    try {
        ...
        // Define read-only data tools
        const searchBookingsTool = async (queryFilter) => { return await Booking.find({ ...queryFilter, hotelId }).limit(50).lean(); };
        const searchRoomsTool = async (queryFilter) => { return await Room.find({ ...queryFilter, hotelId }).populate('roomTypeId').lean(); };
        ...
        const searchCashJournalTool = async (queryFilter) => { return await CashJournal.find({ ...queryFilter, hotelId }).sort({ date: -1 }).limit(30).lean(); };
        ...
        const searchExpensesTool = async (queryFilter) => { return await Expense.find({ ...queryFilter, hotelId }).sort({ date: -1 }).limit(50).lean(); };
        const searchChecklistsTool = async (queryFilter) => { return await Checklist.find({ ...queryFilter, hotelId }).sort({ createdAt: -1 }).limit(30).lean(); };
        ...
        const searchUsersTool = async (queryFilter) => { return await User.find({ ...queryFilter, hotelId }).select('-password').lean(); };
        ...
        const searchGatewaysTool = async (queryFilter) => { return await Gateway.find({ ...queryFilter, hotelId }).select('-consumerKey -consumerSecret').lean(); };
        const searchPaymentTransactionsTool = async (queryFilter) => { return await PaymentTransaction.find({ ...queryFilter, hotelId }).sort({ createdAt: -1 }).limit(50).lean(); };
        ...
        const toolsConfig = [
            {
                functionDeclarations: [
                    ...
                    },
                    {
                        ...
                        parameters: {
                            type: "OBJECT",
                            properties: { name: { type: "STRING", description: "Room category type (e.g., 'Deluxe Room')" } }
                        }
                    },
                    {
        ...
        while (loops < maxLoops) {
            ...
            // 🛡️ FIX: Safely parse content parts directly from the top active candidate object block
            const candidateContent = aiResponse.candidates?.[0]?.content;
            const parts = candidateContent?.parts || [];
            
            // Extract explicit function calls out of parts layout safely
            const functionCalls = parts.filter(p => p.functionCall);

            if (functionCalls.length > 0) {
                // Add the model's call request array block cleanly to memory context
                ...
                for (const part of functionCalls) {
                    ...
                    if (internalFunctions[toolName]) {
                        ...
                        // Sync function answer output into standard conversation payload structure
                ...
                // 🛡️ FIX: Fallback extraction method aligned to official model response wrapper text helper fields
                responseText = aiResponse.text || parts.map(p => p.text).join("");
                break;
            }
        }
