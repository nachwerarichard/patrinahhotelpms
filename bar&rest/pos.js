...
const searchAccounts = async (query) => {
    ...
    try {
        const res = await fetch(`${BASE_URL}/api/pos/client/search?query=${encodeURIComponent(query)}&hotelId=${hotelId}`, {
...
const addCharge = async (description, number, department) => {
    ...
    try {
        ...
        const endpoint = (department === 'Restaurant') ? `${BASE_URL}/api/kitchen/order` : `${BASE_URL}/sales`;
        ...
        if (!res.ok) throw new Error("Transaction failed at main terminal");
